const els = {
  ...
  revealOverlay: document.querySelector("#revealOverlay"),
...
function showReveal(ev, state) {
  ...
  title.textContent = `${ev.challengerName} 质疑了 ${ev.claimerName}`;

  const cards = document.createElement("div");
  cards.className = "reveal-cards";
  (ev.cards || []).forEach((card, i) => {
    const el = document.createElement("div");
    const ok = card === ev.claimedRank || card === "Joker";
    el.className = `rc ${ok ? "good" : "bad"}`;
    el.style.animationDelay = `${0.1 + i * 0.12}s`;
    el.textContent = card === "Joker" ? "★" : card;
    cards.append(el);
  });

  const stamp = document.createElement("div");
  stamp.className = `reveal-stamp ${ev.isLying ? "lie" : "honest"}`;
  stamp.textContent = ev.isLying ? "🤥 撒谎" : "😇 诚实";

  const result = document.createElement("div");
  result.className = "reveal-result";
  const loseText = ev.eliminated ? "出局！" : "失去 1 点生命";
  const who = document.createElement("span");
  who.className = "shake";
  who.textContent = `${ev.loserName} ${loseText}`;
  result.append(document.createTextNode(`声称 ${ev.claimedRank} × ${ev.claimedCount} · `), who);

  box.append(title, cards, stamp, result);
  els.revealOverlay.innerHTML = "";
...
window.addEventListener("keydown", (event) => {
  ...
  if (!els.revealOverlay.classList.contains("hidden")) {
    hideReveal();
  } else if (!els.emotePopover.classList.contains("hidden")) {
    closeEmote();
